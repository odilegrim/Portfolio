...
ggplot(oldvsnew, aes(track.album.name, tempo, color = category)) +
  ...
  labs(y = "Tempo (BPM)", title = "Tempo for BMTH's music", color = "Category") +
  stat_summary(
    geom = "point",
    fun = "mean",
    col = "darkred",
...
"my laptop is failing me and can't make the tempograms"
```
...
### **Timbre based self-similarity matrices** for popular and not so popular songs
...
### **Pitch based self-similarity matrices** for popular and not so popular songs
```{r}
  cyfmh <-
  get_tidy_audio_analysis("0WSa1sucoNRcEeULlZVQXj") %>% # Change URI.
  compmus_align(bars, segments) %>%                     # Change `bars`
